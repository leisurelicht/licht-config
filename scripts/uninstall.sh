...
source "${repo_root}/apps/brew_lists.sh"
# shellcheck source=../apps/for_claude_lists.sh
source "${repo_root}/apps/for_claude_lists.sh"
...
uninstall_apps() {
	...
	brew)
		...
		for item in "${BREW_ITEMS[@]}"; do
			...
			if [[ "${brew_mode}" == "formula" && "${BREW_ITEM_TYPE}" != "formula" ]]; then
				continue
			fi
			if [[ "${brew_mode}" == "cask" && "${BREW_ITEM_TYPE}" != "cask" ]]; then
				continue
			fi

			if [[ "${BREW_ITEM_TYPE}" == "formula" ]]; then
				if brew list --formula "${BREW_ITEM_NAME}" >/dev/null 2>&1; then
					log_step "Uninstall brew formula [ ${BREW_ITEM_NAME} ]"
					brew uninstall "${BREW_ITEM_NAME}" >/dev/null 2>&1 || true
				fi
			else
				if brew list --cask "${BREW_ITEM_NAME}" >/dev/null 2>&1; then
					log_step "Uninstall brew cask [ ${BREW_ITEM_NAME} ]"
					brew uninstall --cask "${BREW_ITEM_NAME}" >/dev/null 2>&1 || true
				fi
			fi
		done
		;;
	claude)
		for item in "${CLAUDE_BREW_ITEMS[@]}"; do
			...
			if [[ -n "${BREW_ITEM_TAP}" ]] && brew tap | grep -Fxq "${BREW_ITEM_TAP}"; then
				log_step "Remove brew tap [ ${BREW_ITEM_TAP} ]"
				brew untap "${BREW_ITEM_TAP}" >/dev/null 2>&1 || true
			fi
		done
		;;
	*)
